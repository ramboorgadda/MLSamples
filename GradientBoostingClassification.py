...
for i in range(len(list(models))):
    ...
    print(f'Accuracy of train {model}: {accuracy_score(y_train, y_trainpred)}')
    print(f'Confusion Matrix of train {model}:\n{confusion_matrix(y_train, y_trainpred)}')
    print(f'Classification Report of train {model}:\n{classification_report(y_train, y_trainpred)}')
    print(f'Precision of train {model}: {precision_score(y_train, y_trainpred)}')
    print(f'Recall of train {model}: {recall_score(y_train, y_trainpred)}')
    print(f'F1 Score of train {model}: {f1_score(y_train, y_trainpred)}\n')
    
    print(f'Accuracy of test {model}: {accuracy_score(y_test, y_testpred)}')
    print(f'Confusion Matrix of test {model}:\n{confusion_matrix(y_test, y_testpred)}')
    print(f'Classification Report of test {model}:\n{classification_report(y_test, y_testpred)}')
    print(f'Precision of test  {model}: {precision_score(y_test, y_testpred)}')
    print(f'Recall of test {model}: {recall_score(y_test, y_testpred)}')
    print(f'F1 Score of test {model}: {f1_score(y_test, y_testpred)}\n')
    print(f'{"-"*50}\n')
...
gradient_param={"loss": ['log_loss','deviance','exponential'], "criterion": ['friedman_mse','squared_error','mse'], "min_samples_split": [2, 8, 15, 20],"n_estimators": [100, 200, 500],"max_depth": [5, 8, 15, None, 10]
                }
random_model =[
                ('RF',RandomForestClassifier(),rf_params),
                ('GradientBoost', GradientBoostingClassifier(), gradient_param)]
print(random_model)
model_params={}
...
models={
    "GradientBoost": GradientBoostingClassifier(n_estimators=500,min_samples_split=20,max_depth=15,loss='exponential',
...
for i in range(len(list(models))):
    ...
    print(f'Roc AUC Score of train {model}: {roc_auc_score(y_train, y_trainpred)}')
    print(f'{"-"*50}\n')
    ...
    print(f'Recall of test {model}: {recall_score(y_test, y_testpred)}')
    print(f'F1 Score of test {model}: {f1_score(y_test, y_testpred)}\n')
    print(f'Roc AUC Score of test {model}: {roc_auc_score(y_test, y_testpred)}')
    print(f'{"-"*50}\n')
...
auc_models = [
{
    ...
    'model': GradientBoostingClassifier(n_estimators=500,min_samples_split=20,max_depth=15,loss='exponential',criterion='squared_error'),
    'auc':  0.9026
},
